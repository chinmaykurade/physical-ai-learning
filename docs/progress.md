...
| Set up Hugging Face account (datasets + hub) | ☑ | 2026-07-26. Account `chinmaykurade`; CLI login verified via `whoami`. **Model push verified end to end 2026-07-26** — 50-step Diffusion Policy uploaded to a private repo via `train_pusht_toy.sh --push`: 1.05 GB (`model.safetensors` + config + both processors + generated card), ~2 min upload; test repo deleted after. Local auth is a cached `hf auth login` token; the pod needs `HF_TOKEN` (write scope) as an env var. Plan §5.2 |
| Set up RunPod (or equivalent) account, ₹2,000–3,000 budget | ☑ | Account created 2026-07-26. Spend starts in Phase C. Pod automation written 2026-07-26 (`scripts/runpod_bootstrap.sh`, `scripts/train_pusht_toy.sh`, `scripts/README.md`) — ssh → bootstrap → train → stop, no manual setup. Base image `runpod/pytorch:1.1.0-cu1300-torch291-ubuntu2404`: `torch==2.11.0` hard-requires the CUDA 13 stack, and no RunPod template ships past torch 2.9.1, so the venv is built from the lock on the `/workspace` volume. Train script verified locally at 50 steps; **not yet run on a pod** |
